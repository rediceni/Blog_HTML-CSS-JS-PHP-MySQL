...
async function readAllPosts() {
  await fetch(`API/post/readAll.php?user=${paramUser}`, {
    method: "GET",
    headers: {
    ...
    .then((data) => {
      if (Object.keys(data)[0] == "danger") {
        ...
        data.forEach((post) => {
          ...
          EditPost.href = `EditPostUser.php?id=${post.id}&user=${paramUser}`;
          ...
          DeletePost.href = `DeletePostUser.php?id=${post.id}&user=${paramUser}`;
